...
class AdminArticlesManagementTest extends TestCase
{
    ...
    private function bulkInsertArticles(int $count, User $author, array $overridesPerRow = []): void
    {
        ...
        // Inserimento a blocchi: singole insert() da migliaia di righe
        // superano placeholder limit/pacchetti massimi lato driver.
    ...
    public function test_the_list_is_paginated_server_side_not_a_single_full_table(): void
    {
        ...
        $response->assertOk();
        // Pagina 1 mostra solo un sottoinsieme (25 per pagina): un titolo
        // tra i più vecchi (fuori dalla finestra "più recenti" ordinata
        // per data decrescente) non deve comparire nella prima pagina.
    ...
    // ── 5. Ogni stato editoriale realmente supportato ─────────────────
    ...
    public function test_combining_search_status_and_category_narrows_correctly(): void
    {
        ...
        // Stesso titolo/parola chiave ma categoria diversa: deve restare escluso.
        $this->article($editor, [
        ...
        // Stessa categoria e parola chiave ma stato diverso: deve restare escluso.
        $this->article($editor, [
    ...
    // ── 7. Persistenza query-string nella paginazione ─────────────────

    public function test_filters_persist_across_pagination_links(): void
    {
        $editor = $this->editor();
        // 30 pubblicati in 'spazio' (due pagine a 25/pagina) + rumore in
        // un'altra categoria che non deve mai comparire, a nessuna pagina.
        ...
        $response->assertOk();
        // Il link "pagina successiva" deve portare con sé il filtro categoria.
    ...
    // ── Sanitizzazione parametri: nulla deve rompere la pagina ────────
    ...
    public function test_a_non_numeric_author_parameter_is_ignored_not_an_error(): void
    {
        $editor = $this->editor();
        $this->article($editor, ['title' => 'Articolo visibile']);

        $response = $this->actingAs($editor)->get(route('admin.articles', ['author' => 'non-un-id']));
    ...
    public function test_a_page_number_beyond_the_last_page_shows_an_empty_page_not_an_error(): void
    {
        $editor = $this->editor();
        $this->article($editor, ['title' => 'Articolo visibile']);
        ...
        $response->assertOk();
        $response->assertDontSee('Articolo visibile');
    }

    public function test_a_search_term_containing_percent_and_underscore_is_treated_literally(): void
    {
        // Senza escaping corretto del LIKE, "50%" verrebbe interpretato
        // come pattern SQL (qualunque cosa preceduta da "50") invece che
        // come testo letterale — questo articolo non contiene "50" da
        // solo, quindi comparirebbe SOLO se il carattere "%" fosse
        // trattato come wildcard invece che come testo cercato.
        $editor = $this->editor();
        $this->article($editor, ['title' => 'Sconto del 50% sugli abbonamenti']);
        $this->article($editor, ['title' => 'Cinquanta ragioni per iscriversi']);

        $response = $this->actingAs($editor)->get(route('admin.articles', ['q' => '50%']));

        $response->assertOk();
        $response->assertSee('Sconto del 50% sugli abbonamenti');
        $response->assertDontSee('Cinquanta ragioni per iscriversi');
    }

    // ── 9. Accesso admin/autorizzazione (contesto filtri) ──────────────
    ...
    // ── 10. Dataset grande: nessuna riga oltre la pagina corrente ──────
    ...
    public function test_with_a_thousand_articles_only_one_pages_worth_is_rendered(): void
    {
        ...
        // Conta le righe <tr> nel corpo tabella: mai più di PER_PAGE (25),
        // qualunque sia la dimensione reale dell'archivio.
    ...
    /**
     * Prova di scala a 10.000 righe, coerente con il resto della codebase
     * (vedi Newsletter2EndToEnd10kConcurrencyTest) — dimostra che la
     * strategia resta valida oltre l'ordine di grandezza minimo richiesto
     * (1.000), non solo nel caso comodo.
     */
