...
{
    public function test_private_paths_are_disallowed_for_all_crawlers(): void
    {
        $groups = $this->robotsGroups();
        $privatePaths = ['/admin/', '/redazione/', '/storage/', '/api/'];

        $rules = $this->rulesForAgent($groups, '*');

        foreach ($privatePaths as $path) {
            $this->assertContains('Disallow: '.$path, $rules);
        }
    }

    public function test_search_is_crawlable_in_robots(): void
    {
    ...
    public function test_robots_advertises_the_complete_sitemap_index(): void
    {
        ...
        $this->assertStringContainsString(
            'Sitemap: https://kairus.it/sitemap-index.xml',
            $robotsTxt
        );
    }
    ...
    public function test_sitemap_index_contains_normal_and_news_sitemaps_once_with_https_urls(): void
    {
        ...
        $this->assertSame(
            1,
            substr_count($xml, '<loc>https://kairus.it/sitemap.xml</loc>')
        );

        $this->assertSame(
            1,
            substr_count($xml, '<loc>https://kairus.it/news-sitemap.xml</loc>')
        );
    ...
    private function robotsGroups(): array
    {
        ...
        foreach ($lines as $line) {
            ...
            if (
                ...
                || str_starts_with(strtolower($line), 'sitemap:')
            ) {
                continue;
            }
            ...
            if (str_starts_with(strtolower($line), 'user-agent:')) {
                $agent = strtolower(
                    trim(substr($line, strlen('User-agent:')))
                );
