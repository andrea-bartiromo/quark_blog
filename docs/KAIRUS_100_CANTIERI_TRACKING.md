...
| 48 | Preview/audit sitemap/ricerca/canonical M&C | in_progress | branch `kairus-cantiere-48-preview-audit-sitemap-mente-comportamento` | — | 10 nuovi test (`ContentClusterAdminPreviewTest`: anteprima admin + audit end-to-end sitemap/ricerca/canonical del pacchetto "Mente e comportamento"); `ContentClusterPublicTest` (6/6, verificato bit-per-bit invariato dopo l'estrazione di `ContentClusterShowPageData`); regressione mirata (ContentCluster 281 test, TrovaEntitySearch+Seo+Sitemap 127 test): 280/281 + 127/127, 1 pre-esistente non correlato (`ContentClusterAutoLifecycleCompletionTest`) | — | 46, 47 |
