...
| 38 | Modello interno "Cosa sappiamo davvero" | blocked | [#595](https://github.com/andrea-bartiromo/quark_blog/pull/595) (non mergiata, in pausa) | — | 22+6 nuovi test dedicati (TrustKnowledgeStatementControllerTest+TrustKnowledgeStatementTest); suite di regressione mirata (Admin, 1128 test): 1126 passati, 2 pre-esistenti (`ProjectModelTest.php:235`, `ProjectTaskControllerTest.php:193`); CI green dopo un fix reale (job mariadb-content-clusters, la nuova migration mancava dalla lista di rollback mirato del workflow) | 1 finding Codex reale (P1) non risolto: `docs/TRUST_LAYER_COSA_SAPPIAMO_DAVVERO_PILOT.md` (missione B-45, programma precedente) vieta esplicitamente "nessuna route pubblica **o migration**" per questo esatto concetto finché non arriva una decisione editoriale umana esplicita — il testo non qualifica la migration come "solo pubblica", quindi limitare le route a `/admin` non basta a soddisfarlo per come è scritto. Non risolvibile autonomamente: sospeso in attesa di decisione dell'utente | — |
