...
| 49 | Categorie esistenti → hub editoriali | in_progress | branch `kairus-cantiere-49-categorie-hub-editoriali` | — | 7 nuovi test (`CategoryCuratorNoteTest`: fallback generico invariato senza nota, resa della nota quando presente, creazione/modifica/rimozione da admin, guest bloccato, anteprima admin coerente); regressione mirata (filtro `Category`, 263 test): 263/263; Pint: passed, `git diff --check`: pulito | — | 1-8 |
