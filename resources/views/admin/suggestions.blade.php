...
@section('title', 'Suggerimenti automatici')

@section('content')

{{-- Avviso criteri editoriali --}}
<div style="background:#fffbeb;border:1px solid #fde68a;border-radius:var(--radius);
  ...
  <ol style="margin:.35rem 0 0 1.2rem;padding:0;color:#78350f;line-height:1.7;">
    ...
    <li>Dopo la pubblicazione, vai su <a href="{{ route('admin.verification') }}" style="color:var(--color-accent);">Verifica editoriale</a> e segna l'articolo come verificato</li>
  </ol>
</div>
...
<div class="admin-topbar">
  ...
  <div style="display:flex;gap:.5rem;align-items:center;">
    ...
    <form method="POST" action="{{ route('admin.suggestions.fetch') }}" style="display:inline;">
  @csrf
...
<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;margin-bottom:1.5rem;">
  ...
  <div style="background:{{ $bg }};border-radius:var(--radius);padding:1rem;text-align:center;">
    <div style="font-family:var(--font-display);font-size:1.8rem;font-weight:900;color:{{ $color }};line-height:1;">{{ $count }}</div>
    <div style="font-family:var(--font-ui);font-size:.7rem;font-weight:600;text-transform:uppercase;letter-spacing:.08em;color:{{ $color }};opacity:.8;margin-top:.25rem;">{{ $label }}</div>
  </div>
  @endforeach
</div>

{{-- Come funziona (primo utilizzo) --}}
  ...
  <div style="background:var(--color-white);border-radius:var(--radius);box-shadow:var(--shadow);overflow:hidden;">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;
      ...
      <div style="display:flex;align-items:center;gap:.75rem;">
        <span class="badge badge--{{ $suggestion->category }}">
          {{ config('laboratorio.categories.'.$suggestion->category) }}
        </span>
        <span style="font-family:var(--font-ui);font-size:.72rem;color:var(--color-ink-muted);">
          📡 {{ $suggestion->source_name }} ·
          {{ $suggestion->fetched_at->diffForHumans() }}
        </span>
      </div>
      <span class="status status--{{ $suggestion->status === 'published' ? 'published' : ($suggestion->status === 'approved' ? 'published' : 'draft') }}"
            style="{{ $suggestion->status === 'rejected' ? 'opacity:.5;' : '' }}">
        {{ $suggestion->status }}
      </span>
    </div>
    ...
    <div style="padding:1.25rem;">
      ...
      {{-- Azioni --}}
      @if($suggestion->status !== 'rejected' && $suggestion->status !== 'published')
      <div style="display:flex;gap:.75rem;align-items:center;flex-wrap:wrap;">

        {{-- Pubblica come bozza --}}
        @if($suggestion->generated_title)
        <button type="button"
                onclick="document.getElementById('publish-form-{{ $suggestion->id }}').classList.toggle('hidden')"
        ...
        {{-- Approva --}}
        <form method="POST" action="{{ route('admin.suggestions.approve', $suggestion) }}" style="display:inline;">
          @csrf
      ...
      {{-- Form pubblicazione (nascosto di default) --}}
      @if($suggestion->generated_title)
      <div id="publish-form-{{ $suggestion->id }}" class="hidden"
        ...
        <form method="POST" action="{{ route('admin.suggestions.publish', $suggestion) }}">
          @csrf
          <div class="form-group">
            <label class="form-label">Titolo</label>
            <input class="form-input" type="text" name="title"
                   value="{{ $suggestion->generated_title }}" required maxlength="255">
          </div>
          <div class="form-group">
            <label class="form-label">Sommario</label>
            <textarea class="form-textarea" name="excerpt"
                      style="min-height:60px;" maxlength="300">{{ $suggestion->generated_excerpt }}</textarea>
          </div>
          <div class="form-group">
            <label class="form-label">Testo articolo</label>
            <textarea class="form-textarea" name="body"
                      style="min-height:250px;" required>{{ $suggestion->generated_body }}</textarea>
          </div>
          <div class="form-group">
            <label class="form-label">Categoria</label>
            <select class="form-select" name="category" required>
              @foreach(config('laboratorio.categories') as $slug => $label)
                <option value="{{ $slug }}" {{ $suggestion->category === $slug ? 'selected' : '' }}>
                  {{ $label }}
                </option>
              @endforeach
            </select>
      ...
      @elseif($suggestion->status === 'published' && $suggestion->article)
      <div style="display:flex;gap:.75rem;">
        <a href="{{ route('admin.articles.edit', $suggestion->article) }}"
           class="btn btn--outline" style="color:var(--color-ink);border-color:var(--color-border);font-size:.78rem;">
          Vai all'articolo →
        </a>
        <a href="{{ route('articolo', $suggestion->article->slug) }}" target="_blank"
