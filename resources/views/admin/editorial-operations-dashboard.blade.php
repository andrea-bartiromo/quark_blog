...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1rem;margin-bottom:1.5rem;">
  ...
  <div style="background:var(--color-white);border-radius:var(--radius);box-shadow:var(--shadow);padding:1.25rem;">
    ...
    <a href="{{ route('admin.second-read') }}" style="font-size:.78rem;color:#0d9488;">{{ number_format($snapshot['second_read']['second_reads']) }} seconde letture su {{ number_format($snapshot['second_read']['source_articles_engaged']) }} articoli sorgente — Dettaglio →</a>
  </div>

  <div style="background:var(--color-white);border-radius:var(--radius);box-shadow:var(--shadow);padding:1.25rem;">
    <div style="font-family:var(--font-ui);font-size:.7rem;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:#6b7280;">Search Console</div>
    @if($snapshot['search_console']['available'])
      <div style="font-size:1.9rem;font-weight:700;margin:.35rem 0;">{{ $snapshot['search_console']['days_since_last_import'] }}</div>
      <a href="{{ route('admin.search-opportunities') }}" style="font-size:.78rem;color:#0d9488;">{{ $snapshot['search_console']['days_since_last_import'] === 1 ? 'giorno dall\'ultimo import' : 'giorni dall\'ultimo import' }} — Dettaglio →</a>
    @else
      <div style="font-size:1.4rem;font-weight:700;margin:.35rem 0;color:#b45309;">Mai importato</div>
      <a href="{{ route('admin.search-opportunities.import-form') }}" style="font-size:.78rem;color:#0d9488;">Importa un export CSV →</a>
    @endif
  </div>
...
<section style="background:var(--color-white);border-radius:var(--radius);box-shadow:var(--shadow);padding:1.25rem;margin-bottom:1.25rem;">
  <h2 style="font-size:1rem;margin:0 0 .75rem;">Percorsi con membri non pubblicabili</h2>
  @if(empty($snapshot['percorsi_non_publishable_members']))
    <p style="font-size:.82rem;color:#6b7280;margin:0;">Nessun Percorso elenca un articolo ancora in bozza o revisione come membro.</p>
  @else
    <ul style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:.4rem;">
      @foreach($snapshot['percorsi_non_publishable_members'] as $row)
        <li style="font-size:.85rem;">
          <a href="{{ route('admin.content-clusters.edit', $row['cluster_id']) }}">{{ $row['name'] }}</a>
          <span style="color:#6b7280;"> —
            @foreach($row['members'] as $member)
              {{ $member['title'] }} ({{ ucfirst($member['status']) }}){{ ! $loop->last ? ', ' : '' }}
            @endforeach
          </span>
        </li>
      @endforeach
