...
final class DashboardDataExportService
{
    ...
    private function secondRead(ExportWindow $window): array
    {
        ...
        return [
            ...
            'schema' => ['period', 'source', 'percorso', 'impressions', 'second_reads', 'second_read_rate', 'sample_size', 'status'],
            'rows' => [[
                'period' => $window->from->toDateString().' / '.$window->to->toDateString(),
                'source' => 'article_continuation_events',
                'percorso' => 'sitewide',
                'impressions' => $available ? $totals['impressions'] : null,
                'second_reads' => $available ? $totals['second_reads'] : null,
